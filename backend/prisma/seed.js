...
async function main() {
  const defaults = [
    ...
    { key: 'maxExposurePct',          value: '150' },
    { key: 'cooldownEntries',         value: '3' },
    { key: 'cooldownWindowMin',       value: '10' },
    { key: 'cooldownDurationMin',     value: '5' },
    { key: 'requireVolumeGate',       value: 'true' },
    { key: 'replacementEnabled',      value: 'true' },
    { key: 'exposureCheckEnabled',    value: 'true' },
    { key: 'paperTrading',            value: 'true' },
    { key: 'tradingEnabled',          value: 'false' },
    { key: 'binanceTestnet',          value: 'false' },
  ];

  for (const d of defaults) {
    await prisma.botConfig.upsert({ where: { key: d.key }, update: {}, create: d });
  }
  console.log(`✓ Default configuration seeded (${defaults.length} keys)`);

  // Seed initial paper balance if no ledger entries exist
  const ledgerCount = await prisma.balanceLedger.count({ where: { mode: 'PAPER' } });
  if (ledgerCount === 0) {
    ...
    console.log('✓ Initial paper balance seeded ($10,000)');
  }
}
