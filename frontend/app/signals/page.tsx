      ...
      {!liveMode && (
        <>
          {isLoading ? (
            ...
            <div className="card" style={{ padding: 40, textAlign: 'center', color: 'var(--text3)' }}>No signals recorded yet</div>
          ) : (
            <>
              {/* Desktop table */}
              ...
              <div className="mobile-only">
                {sigArr.map((s: any) => (
                  <div key={s.id} className="m-card">
                    {/* Header: symbol + direction + acted */}
                    <div className="m-row">
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <span style={{ fontWeight: 700, fontSize: 14 }}>{s.symbol}</span>
                        <DirectionBadge dir={s.direction} />
                      </div>
                      {s.acted ? <span className="badge-green">ACTED</span> : <span className="badge-gray">NO ACTION</span>}
                    </div>
                    ...
                    {/* Momentum */}
                    <div>
                      <div className="m-label" style={{ marginBottom: 6 }}>Momentum Score</div>
                      <MomentumBar score={s.momentumScore} />
                    </div>
                    ...
                    <div className="m-row" style={{ marginTop: 10 }}>
                      <div>
                        ...
                        <div className="m-val">{((s.confidence || 0) * 100).toFixed(0)}%</div>
                      </div>
                      <div>
                        <div className="m-label">Trend</div>
                        <div style={{ fontSize: 12, color: s.trendDirection === 'BULLISH' ? 'var(--green)' : s.trendDirection === 'BEARISH' ? 'var(--red)' : 'var(--text2)' }}>
                          {s.trendDirection || '—'}
                        </div>
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        <div className="m-label">Vol Ratio</div>
                        <div style={{ fontSize: 12, color: s.volumeRatio > 1.5 ? 'var(--green)' : 'var(--text2)' }}>
                          {(s.volumeRatio || 0).toFixed(2)}x
                        </div>
                      </div>
                    </div>
                    ...
                    {s.breakoutType && (
                      <div style={{ marginTop: 6 }}>
                        <span className="m-label">Breakout: </span>
                        <span style={{ fontSize: 12, color: s.breakoutType === 'BULLISH' ? 'var(--green)' : 'var(--red)' }}>
                          {s.breakoutType}
                        </span>
                      </div>
                    )}
